...
|---|---|---|
| OpenRouter / model-agnostic routing | Adds a normalization layer on top of an unproven loop | After step 1 is stable — swap single-model call for router, loop logic untouched |
...
| 1 — Real model, fake tools | in progress — local gates pass; live API gate pending |
