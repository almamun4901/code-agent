...
- Mutating calls use host-generated operation IDs, a durable host-owned E2B
  ...
  ADR 0013.
- Production turns persist validated plan/action intent and a stable operation
...
- Result delivery uses a deterministic `result/<run-id>` branch and a bounded
  ...
  then permits sandbox cleanup. Delivery failure preserves the sandbox lease.
- Sandbox task artifacts use the shared task group with a group-writable umask,
  preserving shell/typed-tool edit parity without weakening host isolation.
...
- [x] Explicitly authorize provider training for the synthetic free-model
  acceptance gate. No repository code was sent; production defaults still
...
- Verification: `bun run typecheck`, `bun run test`, `git diff --check`, the
  ...
  reviews were resolved. After loading the repository `.env`, live E2B proved
  cancellation cleanup, interactive revise → restart → approve with delivery,
  and explicit auto approval with delivery. The live run also exposed and
  resolved Anthropic's aggregate execution-schema compilation limit while
  preserving strict host validation.
- Current status of the step in progress: Step 8B is complete after 271 offline
  tests, typecheck, review, merge verification, three live E2B approval paths,
  sandbox cleanup, result delivery, and main push.
- Next session should start with: Plan Gate 8C completion evidence and durable
  host inspection from updated `main`.
...
- What was done: Extracted the provider-neutral model contract, propagated
  ...
  requires parameter-supporting routes, defaults to no-data-collection,
  bounds latency and response size, normalizes cancellation, and fails closed
  on malformed or inconsistent responses. Added deterministic coverage and an
  explicit live multi-turn tool-call gate. Updated current architecture
  documentation.
- What broke / had to be reworked: Review found that a failing response-stream
  ...
  validation and safely normalizes canonical in-band error types. Provider
  data policy is now an exact validated configuration with a secure `deny`
  default and an explicit `allow` override for the authorized synthetic gate.
...
### 2026-07-30 — Headless production runner

- What was done: Added the host-side production runner, version-2 production
  checkpoint, canonical repository/task run identity, routed model loop, E2B
  session ownership, stable pending mutation operation IDs, and cleanup
  reconciliation. Added deterministic coverage for combined and sequential
  plan/action turns, provider-native action chains, bounded full-plan
  revisions, cancellation resume, mismatched checkpoints, and unsafe state
  paths. Recorded ADR 0014 and added an opt-in live acceptance gate.
- What broke / had to be reworked: The free route emits plan and repository
  tools sequentially and revises its plan as observations arrive, so the
  initial parallel/stable-plan protocol was incompatible. The runner now
  accepts one action per provider turn against the last committed plan and
  bounds completion advancement to successful observations. Repeated
  diagnostics then exhausted OpenRouter's daily free request quota.
- Decisions made this session: Persist the validated assistant turn and stable
...
### 2026-08-03 — Transactional result delivery

- What was done: Added shared-group artifact permissions and transactional
  result delivery from E2B into a deterministic local `result/<run-id>` branch.
  The host now exports a bounded delta bundle, validates its ancestry, object
  graph, modes, and protected paths, imports it without checkout, persists a
  recovery receipt, and only then allows cleanup. The CLI reports the delivered
  branch and commit. Crash-point, adversarial, runtime, session, and live E2B
  coverage was added, along with an implementation review and delivery docs.
  A real Anthropic calculator run delivered commit `30e21fd29c16`; all 26 tests
  passed from a detached local worktree after its E2B sandbox was gone.
- What broke / had to be reworked: The first template build inherited an old
  local tag and was rebuilt explicitly as `result-delivery-v1`. Review found
  that no-change runs produced an unusable empty bundle, symlink/gitlink and
  control-character paths needed explicit rejection, and delivery failure
  needed to close MCP while retaining the sandbox lease; all three cases were
  fixed and covered. The installed gstack review bundle lacked its mandatory
  checklist, so the same audit was completed manually and recorded. One branch
  push encountered a transient GitHub 503 and succeeded on retry.
- Decisions made this session: Delivery implements ADR 0020 with a bounded Git
  artifact and durable `exported -> validated -> imported -> completed`
  transitions. Empty successful work receives a completion commit. The host
  refuses dirty repositories and branch collisions, never switches the user's
  branch, and preserves the sandbox for recovery when delivery cannot finish.
- Current status of the step in progress: Gate 8A is complete on merged `main`.
  Post-merge typecheck, 254 offline tests, 36 focused sandbox tests, manual
  preflight, fake-loop recovery, both focused live E2B gates, and cleanup checks
  pass. No E2B sandboxes remain running.
- Next session should start with: Pull updated `main`, create a fresh branch for
  Gate 8B, and design the durable `awaiting_approval` state. Read-only discovery
