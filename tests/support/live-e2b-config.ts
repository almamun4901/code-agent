const REQUIRED_TEMPLATE_TAG = "step-6-v1";
...
export function readLiveE2bConfig(
  ...
  if (!templateRef) {
    throw new Error(
      "RUN_LIVE_E2B_TEST=1 requires E2B_TEMPLATE_ID. Use the tagged templateRef/name returned by the Step 6 template build.",
    );
  }
  if (!templateRef.endsWith(`:${REQUIRED_TEMPLATE_TAG}`)) {
    throw new Error(
      `E2B_TEMPLATE_ID must be a tagged Step 6 reference ending in :${REQUIRED_TEMPLATE_TAG}. The build's bare templateId defaults to the unrelated :default tag; use its templateRef/name value instead.`,
    );
  }
