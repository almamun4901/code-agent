...
describe("E2B runtime template", () => {
  test("pins the runtime and includes every required tool dependency", () => {
    ...
    expect(TOOL_RUNTIME_VERSION).toBe("completion-evidence-v1");
    ...
    expect(dockerfile).toContain("PLAYWRIGHT_BROWSERS_PATH=/opt/agent/ms-playwright");
  ...
  test("creates a deterministic non-secret runtime manifest", async () => {
    ...
    expect(first).toMatchObject({
      runtimeVersion: "completion-evidence-v1",
  ...
  test("reports and requires a tagged runnable template reference", () => {
    expect(
      templateBuildOutput({
        ...
        buildId: "build-id",
        name: "terminal-coding-agent-tools:completion-evidence-v1",
      }),
    ).toEqual({
      templateId: "template-id",
      templateRef: "terminal-coding-agent-tools:completion-evidence-v1",
      buildId: "build-id",
      name: "terminal-coding-agent-tools:completion-evidence-v1",
    });
  ...
  test("converts millisecond tool timeouts to GNU timeout seconds", async () => {
    ...
    expect(wrapper).toContain("pkill -KILL -u runner");
